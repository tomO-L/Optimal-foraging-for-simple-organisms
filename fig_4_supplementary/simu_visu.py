...
for n in range(l):
    ...
    t = schedule[n]
    rho_depleted = depletion(rho_final[n:n + n_r], [t_d0, rho_s, t])
...
for i in range(l):
    eta_m_list.append(r/n_r*np.sum(gamma_of_rho(rho_final[i:i + n_r],t_d0, rho_s)))
...
#axes[0].set_xlabel(r'Position ($S$)',fontsize=axes_font_size)
...
# plot([zoom_region[_] for _ in [0, 1, 1, 0, 0]], [zoom_region[_] for _ in [2, 2, 3, 3, 2]], color="black", linewidth=1.5)

axes.set_xlim(min(x),max(x))
axes.set_ylim(.5, 1.5*v_lim)
axes.set_yscale('log')
#axes[1].set_xlabel(r'Position ($S$)',fontsize=axes_font_size)
...
axes.set_yscale('log')
#axes[1].set_xlabel(r'Position ($S$)',fontsize=axes_font_size)
# axes.set_xlabel('$S$', fontsize=axes_font_size)
axes.set_xticks([])
...
axes.set_xlim(zoom_region[0], zoom_region[1])
axes.set_ylim(zoom_region[2], zoom_region[3])
# axes.set_xlabel(r'$S$',fontsize=axes_font_size)
