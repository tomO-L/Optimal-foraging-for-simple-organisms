...
def simu_rough(rho_init, T, t_d0, n_r, r, v_lim, eta_target):
    ...
    while n + n_r < length:
        ...
        eta_test = np.sum(rho_depleted) * v 
...
def simu(rho_init, T, t_d0, n_r, r, v_lim, eta_target, noise=0, learning=False, trade_off=False, alpha=0, a=1):
    ...
    nb_of_corrections = 1
    CHI_before = np.inf
    tolerance = 1 # For Alfonso to play with
    k = 100 # Elasticity. For Alfonso to play with
    ...
    for iteration in tqdm.tqdm(range(nb_of_corrections)):
        ...
        while abs(chi_local) >= tolerance and chi.size!=0:
            ...
            chi_search = eta_m_list-eta_target - k*neighbour_diff(schedule)
            ...
            chi = chi_search[points_of_interest]
            CHI = np.sum(abs(chi_search))
            ...
            if chi.size!=0:
                ...
                new_CHI = np.sum(abs(verif_eta_m_list - eta_target - k*neighbour_diff(schedule)))          
...
if __name__=='__main__':
    ...
    os.makedirs(os.path.join(script_dir, name), exist_ok=True )
