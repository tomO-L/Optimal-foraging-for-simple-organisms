...
def simu(rho_init, T, t_d0, n_r, r, v_lim, eta_target, noise=0, learning=False, trade_off=False, alpha=0, a=1):
    ...
    #rho_init = np.ones(len(rho_init))*10
    ...
    def compute_eta_m(rho):
        ...
        for n in range(length):
            ...
            eta_m.append(r/n_r*np.sum(gamma_of_rho(rho[n:n + n_r],t_d0,rho_s)))
    ...
    nb_of_corrections = 1 # For Alfonso to play with # Set it to 1 if you are running eta_star_vs_eta_bar.py and 30 instead
    CHI_before = np.inf
    ...
    for iteration in tqdm.tqdm(range(nb_of_corrections)):
        ...
        tolerance = 1 #0.1 for simulations that are not shown, 1 instead. For Alfonso to play with
        n = 0
        chi_local = np.inf
        chi = np.array([0])
...
if __name__=='__main__':
    ...
    #density_eaten = depletion(rho[:-1], [t_d0, 5, time_felt])[tail*2:-tail-1]
    #density_eaten = depletion(rho[:-1], [t_d0, 5, time_felt])[tail*2:-tail-1]
