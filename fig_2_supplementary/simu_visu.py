...
for n in range(l):
    ...
    t = schedule[n]
    rho_depleted = depletion(rho_final[n:n + n_r], [t_d0, 5, t])
    rho_final[n:n + n_r] = rho_final[n:n + n_r] - rho_depleted
    eta = np.sum(rho_depleted)*r/n_r/t
    eta_list.append(eta)

for i in range(l):
    eta_m_list.append(r/n_r*np.sum(gamma_of_rho(rho_final[i:i + n_r],t_d0,5)))
...
eta_bar = np.sum(rho - rho_final)*r/n_r/np.sum(schedule)
v = r/n_r/schedule
